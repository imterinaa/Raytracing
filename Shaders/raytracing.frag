...
const vec3 Unit = vec3 ( 0.0, 0.0, 1.0 );
out vec4 FragColor;
in vec3 glPosition;
...
{
    light.Position = vec3(0.0, 2.0, -3.0f); 
...
void main ( void )
{
	...
	uCamera.Position = vec3(0.0,0.0,-11.0);
		...
		{   
    		switch(intersect.MaterialType){
				...
				{  
					...
					resultColor += trRay.contribution * Phong ( intersect, uLight, -normalize(ray.Direction), shadowing );   
					break;       
				}  
				...
				case REFRACTION:
				{
					...
					if (fromOutside){
						refractDirection = normalize( refract(ray.Direction, intersect.Normal,1/1.3));
						contribution = 0.1;
					}
					else {
						refractDirection = normalize( -refract(ray.Direction, intersect.Normal, 1.3));
						contribution = 0.7;
					}
